...
ii rec(int n, int m, int inv, int ent){
//	cout << n << " " << m << " " << inv << endl;
	if(n==1) return {1, 1};

	
	ii ret; int m1=inv, m2=inv;
	if(ent == 0)  m1 = (m1+1)%4, m2 = (m2+3)%4;
	else  m1 = (m1+3)%4, m2 = (m2+1)%4;
	...
	if(inv==0 and ent==1){
		if(m<=n*n/4) ret.f += n/2;		
		else if(m<=n*n/2) ret.f += n/2, ret.s += n/2;
		else if(m <=n*n/4*3) ret.s += n/2;
		else if(m<=n*n){}
	}
	if(inv==1 and ent==0){
		if(m<=n*n/4) ret.s += n/2;	
		else if(m<=n*n/2) ret.f += n/2, ret.s += n/2;
		else if(m <=n*n/4*3) ret.f += n/2;
		if(m<=n*n){}
	}
	if(inv==1 and ent==1){
		if(m<=n*n/4){}
	...
	if(inv == 2 and ent==1){
		...
		else if(m<=n*n) ret.f += n/2, ret.s += n/2;
	}
	if(inv == 3 and ent==0){
		if(m<=n*n/4) ret.f += n/2;			
		else if(m<=n*n/2) {}
