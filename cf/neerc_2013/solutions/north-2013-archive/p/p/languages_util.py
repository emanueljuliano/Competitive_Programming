...
def runCompiled(program, args):
    return ["./" + util.replaceSuffix(program, (".exe" if util.windows else ""))] + args
...
def java(classpath = [], args = []):
    return ["java"] + defaults.javaOptions + joinClasspath(classpath) + list(args)
...
def javaToJar(program):
    ...
    try:
        ...
        if name == "Check":
            os.chdir(temp)
            util.execute("jar", "xf", util.relpath("testlib/testlib4j.jar"))
            os.chdir("..")
            manifest = [
        ...
        if name == "Validate" and os.path.exists(util.relpath("testlib/validate4j.jar")):
            os.chdir(temp)
            util.execute("jar", "xf", util.relpath("testlib/validate4j.jar"))
            os.chdir("..")
            manifest = [
