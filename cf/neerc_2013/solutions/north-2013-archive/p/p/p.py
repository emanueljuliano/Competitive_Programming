...
class Test:
    ...
    def run(self, options, action, solution):
        ...
        if exitcode:
            s = " (" + runExitCodes[exitcode] + ")" if exitcode in runExitCodes else ""
    ...
    def check(self, action):
        ...
        with open("check.__", "w") as stdout:
            result = run("Check", self.problem.inputFile, self.problem.outputFile, self.answer, stdout=stdout)
            output = ' '.join([line.strip() for line in util.readLines("check.__")])
            print(output)
            if result:
                self.logAction(action, "Check has not accepted solution: {}".format(output), log.warning)
                return 1
        self.logAction(action, "ok", log.info)
        return 0

runExitCodes = {
    0xFFFFFFFF: "TIME_LIMIT_EXCEEDED",
    0xFFFFFFFE: "MEMORY_LIMIT_EXCEEDED",
    0xFFFFFFFD: "IDLENESS_LIMIT_EXCEEDED",
    0xFFFFFFFC: "CRASH",
    0xFFFFFFFB: "SECURITY_VIOLATION",
    0xFFFFFFFA: "WRONG_ANSWER",
    0x80000000: "FAIL",
}
...
def runAction(options, action, testAction):
    ...
    exitcode = functools.reduce(
        (lambda success, test :
            ...
            testAction(options, test, solution) or success),
        tests,
        0
    )
...
parser = argparse.ArgumentParser(
    ...
    epilog = """
        ...
        \\n* date -- contest date
        \\n* time-limit -- default problem time limit
        \\n* memory-limit -- default problem memory limit
...
def time(options, program, *args, stdin = None, stdout = None):
    ...
    timeLimit = options.time_limit if options.time_limit else defaults.timelimit
    memoryLimit = options.memory_limit if options.memory_limit else defaults.memorylimit
...
def banner(problemDir, comments, file, type = None, author = None):
    ...
    util.writeFile(util.join(problemDir, file), "{}", "\n".join([
            line,
            text("{}", contest["name-1"]),
            text("{}", contest["name-2"]),
...
def problem_validate(options):
    ...
    for test in tests:
        with open(test.input) as stdin:
            ...
            if exitcode:
                raise PException("Validate exit code {} on test {}", exitcode, test)
    return 0
