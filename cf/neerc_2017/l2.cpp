...
int main(){ _
	...
	for (auto [a, b] : query) {
		...
		if (hld::query_path(a) != 0) pcolor = hld::query_path(a), contained = true;
		hld::update_path(a, b, pcolor, v);
		for (auto [l, r, c] : v) {
		//	cout << l << " " << r << " " << c << endl;
			if (contained and c != pcolor) can = false;
			else if (!contained and c != 0) can = false;
		}
	}
