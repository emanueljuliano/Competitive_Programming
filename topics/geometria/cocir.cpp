		...
		for(int i=0;i<n; i++){
			for(int j=i+1; j<n; j++){
				for(int k=j+1; k<n; k++){
					...
					int tmp =3;
					pt ce = getcenter(a, b, c);
					ld r = dist(a, ce);
					//st.insert(r);
					int ini = lower_bound(w.begin(), w.end(), ce.x-r) - w.begin();
					int endi = upper_bound(w.begin(), w.end(), ce.x+r) - w.begin();
					for(int l=ini; l<endi; l++){
						if(l==i or l==j or l==k) continue;
						if(eq(dist2(ce, v[l]), sq(r))) tmp++;
					}
					ans = max(ans, tmp);
				}	
			}
