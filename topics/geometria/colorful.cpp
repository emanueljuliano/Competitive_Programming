...
bool con(int i, int j){
	...
	bool ret =  dist2(q, p) < sq(r+s);
	if(ret){
		return sqrt(dist2(q, p)) + double(min(r, s)) > (double)max(r, s) + 1e-9;
	}
	return ret;
}
...
int main(){ _
	...
	if(n==2){
		...
		int sz = st.size();
//		cout << sz << endl;
		int doi = 0; doi += v1.size()>1; doi += v2.size()>1; doi+= v3.size()>1;
		if(sz==6) ans+=4;
		if(sz==5) ans += 3;
		if(sz==4){
			ans += 2;
