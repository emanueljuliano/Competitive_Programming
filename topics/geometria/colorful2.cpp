...
int n;
vector<pair<pt, int>> C;


const int MAX = 2*50*50 + 10;
...
void build(int m){
	for(int i=0;i <m; i++) id[i]=i, sz[i]=1, vis[i]=0;
}
...
int main(){ _
	...
	for(int i=0;i <n; i++){
		...
		for(int j=0; j<n; j++){
			if(i==j) continue;
			vector<pt> v = circ_inter(C[i].f, C[j].f, C[i].s, C[j].s);
			for(auto k : v) st.insert(k), curr.insert(k);
		}
		if(!curr.empty()){
			e+=curr.size()+1;
			ver++;
	...
	int comp = 0;
	for(int i=0;i <n; i++){
		if(!vis[find(i)]) vis[find(i)]=1, comp++;
	}
	ans += e - ver + comp;
	cout << ans << endl;
