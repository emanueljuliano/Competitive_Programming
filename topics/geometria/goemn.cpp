...
double ang;
struct cmp {
	bool operator () (const pair<line, int>& aa, const pair<line, int>& bb) {
		line a = aa.f, b=bb.f;
		line r = line(pt(0, 0), rotate(pt(1, 0), ang));
		return getm(a) < getm(b);
		//return norm(inter(r, a)) < norm(inter(r, b));
	}
};

set<pt> st;
bool ord(pair<line, int> aa, pair<line, int> bb){
	if(eq(getm(aa.f), getm(bb.f))) st.insert(bb.f.q), st.insert(aa.f.q);
	return getm(aa.f) < getm(bb.f);
...
int main(){ _
	...
	while(t--){
		...
		for(int i=0;i <g; i++){
			ld x, y; cin >> x >> y;
			line r = line(pt(0, 0), rotate(pt(0, 1), ang));
			line l = line(e, pt(x, y));
			pt inte = inter(r, l);
			pair<ld, int> pa = {inte.y, -1};
			int up = upper_bound(ev.begin(), ev.end(), pa) - ev.begin();
			int lo = lower_bound(ev.begin(), ev.end(), pa) - ev.begin();
			if(!eq(ev[lo].f, pa.f) and !eq(ev[up].f, pa.f)){
				if(ev[lo].s==1) ans++;
			}
		}
