...
ld angle(pt b){
	ld ret = atan2(b.y, b.x);
	while(ret<0) ret += 2*pi;
	return ret;
}

bool ord(pair<pair<pt, int>, line> a, pair<pair<pt, int>, line> b){
	return angle(a.f.f) < angle(b.f.f);
}

bool ant(pt a, line l){
	line r = line(pt(0, 0), rotate(pt(1, 0), ang));
	return norm(a) < norm(inter(r, l));	
...
int main(){ _
	...
	while(cin >> s >> k>> w){
		...
		for(int i=0;i <w; i++){
			...
			W.pb(line(pt(a, b), pt(c, d)));
		}

		for(int t=0;t <s; t++){
		//	cout<< "Seeker "  << S[t].x << " " << S[t].y << endl;
			int ans = 0;
			vector<pair<pair<pt, int>, line>> ev;

			for(int j=0; j<s; j++){
				if(j!=t) ev.pb({{S[j]-S[t], 0}, line()});
			}
			for(int j=0; j<k; j++){
				ev.pb({{K[j]-S[t], 0}, line()});
			}

			for(int j=0; j<w; j++){
				W[j].p = W[j].p - S[t], W[j].q = W[j].q - S[t];
				ld wa = angle(W[j].p);
				ld wb = angle(W[j].q);
			//	cout << "linha " << W[j].p.x << " " << W[j].p.y<< " " <<  W[j].q.x <<" "<<  W[j].q.y; 	
				pt aux =  inter(W[j], line(pt(0, 0), pt(DINF, 0)));
			//	cout << " inter " << aux.x <<"  " << aux.y << " ";
				if(interseg(W[j], line(pt(0, 0), pt(DINF, 0)))){
			//		cout << " seg" << endl;
					if(wa<wb) swap(W[j].p, W[j].q);
					ev.pb({{W[j].p, 1}, W[j]});
					ev.pb({{W[j].q, 2}, W[j]});
			...
			set<line, cmp> st;
			for(int i=0;i <2; i++){ ans = 0;	
			for(auto IT : ev){
				pt p = IT.f.f;
				int id = IT.f.s;
				line l = IT.s;
				ang = angle(p)+eps;
			//	if(i)cout << id << " p= " << p.x << " " << p.y << " st= " << st.size() <<  endl;
